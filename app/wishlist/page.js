...
const initialWishlist = [
  { id: "k1",  name: "Keychron Q1 Pro Wireless",       brand: "Keychron",  category: "คีย์บอร์ด",     price: 5990,  originalPrice: 7500,  rating: 4.9, reviews: 412,  img: "https://images.unsplash.com/photo-1618384887929-16ec33fab9ef?w=500&q=80", tags: ["Wireless","Hot-swap","75%"],       inStock: true,  href: "/category/keyboard" },
  { id: "m1",  name: "Logitech G Pro X Superlight 2",  brand: "Logitech",  category: "เมาส์",          price: 5490,  originalPrice: 6990,  rating: 4.9, reviews: 1024, img: "https://images.unsplash.com/photo-1527814050087-3793815479db?w=500&q=80", tags: ["61g","Wireless","32,000 DPI"],     inStock: true,  href: "/category/mouse" },
  { id: "h2",  name: "Sony WH-1000XM5",                brand: "Sony",      category: "หูฟัง & ลำโพง", price: 9990,  originalPrice: 12990, rating: 4.9, reviews: 2841, img: "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=500&q=80", tags: ["ANC","Bluetooth","30hr"],         inStock: true,  href: "/category/headset" },
  { id: "mo1", name: "ASUS ROG Swift PG27AQDM OLED",   brand: "ASUS ROG",  category: "Monitor",        price: 32900, originalPrice: 38900, rating: 4.9, reviews: 412,  img: "https://images.unsplash.com/photo-1616763355548-1b606f439f86?w=500&q=80", tags: ["OLED","QHD","240Hz"],             inStock: false, href: "/category/monitor" },
  { id: "s1",  name: "Samsung 990 Pro NVMe 2TB",       brand: "Samsung",   category: "Storage & SSD",  price: 4290,  originalPrice: 5990,  rating: 4.9, reviews: 2341, img: "https://images.unsplash.com/photo-1597872200969-2b65d56bd16b?w=500&q=80", tags: ["NVMe","PCIe 4.0","7,450 MB/s"],  inStock: true,  href: "/category/storage" },
];
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-md mx-auto px-4 py-24 flex flex-col items-center text-center">
        ...
        <p className="text-gray-400 text-sm mb-8 leading-relaxed">
          คุณต้องเข้าสู่ระบบก่อนเพื่อดูรายการโปรดของคุณ<br />
          บันทึกสินค้าที่ชอบไว้ดูภายหลังได้เลย
        </p>
        <div className="flex flex-col sm:flex-row gap-3 w-full">
          <Link href="/login" className="flex-1 text-center bg-amber-400 hover:bg-amber-300 text-gray-950 font-bold py-3 rounded-xl text-sm transition-all hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(251,191,36,0.3)]">เข้าสู่ระบบ</Link>
          <Link href="/register" className="flex-1 text-center border border-white/15 hover:bg-white/5 text-white font-medium py-3 rounded-xl text-sm transition-colors">สมัครสมาชิกฟรี</Link>
        </div>
        <div className="mt-10 w-full border-t border-white/8 pt-8">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {[
              ...
              <Link key={c.href} href={c.href} className="text-xs px-3 py-1.5 rounded-full border border-white/15 text-gray-400 hover:border-amber-400/50 hover:text-amber-400 transition-colors">
                {c.label}
              </Link>
            ))}
          </div>
...
export default function WishlistPage() {
  ...
  return <WishlistContent session={session} userData={userData} />;
}

// ─── Wishlist Content ─────────────────────────────────────────
function WishlistContent({ session, userData }) {
  ...
  const displayName = userData?.name || session.user?.user_metadata?.name || session.user?.email || "คุณ";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
          <div>
            ...
            {/* ── ชื่อจริงจาก Supabase ── */}
            <p className="text-gray-500 text-sm mt-1">สวัสดี, <span className="text-amber-400">{displayName}</span></p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6">
            ...
            <div className="flex items-center gap-2 ml-auto">
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
                ...
                {["เพิ่มล่าสุด","ราคา: ต่ำ → สูง","ราคา: สูง → ต่ำ","คะแนนสูงสุด"].map((s) => <option key={s}>{s}</option>)}
              </select>
              <button onClick={() => setWishlist([])} className="text-xs text-gray-500 hover:text-red-400 transition-colors px-2 py-1.5">
                ลบทั้งหมด
              </button>
            </div>
          </div>
        ...
        {wishlist.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <Link href="/" className="bg-amber-400 hover:bg-amber-300 text-gray-950 font-bold px-8 py-3 rounded-xl text-sm transition-all hover:-translate-y-0.5">
              เริ่มช้อปเลย
            </Link>
          </div>
        )}
            ...
            {sorted.map((p) => (
              <div key={p.id}
                ...
                <div className="relative overflow-hidden bg-gray-800 h-48">
                  ...
                  {!p.inStock && (
                    <div className="absolute inset-0 bg-gray-950/50 flex items-center justify-center">
                      <span className="bg-gray-800 text-gray-400 text-xs font-semibold px-3 py-1.5 rounded-full border border-white/10">สินค้าหมด</span>
                    </div>
                  )}
                ...
                <div className="p-4">
                  ...
                  <Link href={p.href}>
                    <h3 className="text-white text-sm font-semibold leading-snug mb-2 line-clamp-2 hover:text-amber-400 transition-colors">{p.name}</h3>
                  </Link>
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-green-400 text-[10px] font-semibold">ลด ฿{(p.originalPrice - p.price).toLocaleString()}</span>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => handleAddToCart(p.id)} disabled={!p.inStock}
                      ...
                      {addedToCart === p.id
                        ? <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M5 13l4 4L19 7"/></svg>เพิ่มแล้ว</>
                        : p.inStock
                          ? <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>ใส่ตะกร้า</>
                          : "สินค้าหมด"
                      }
                    </button>
                    <button onClick={() => handleRemove(p.id)}
        ...
        {wishlist.length > 0 && sorted.length === 0 && (
          <div className="text-center py-16 text-gray-500">
            ...
            <button onClick={() => setFilterCategory("ทั้งหมด")} className="mt-2 text-xs text-amber-400 hover:underline">ดูทั้งหมด</button>
          </div>
        )}
      </div>
    </div>
